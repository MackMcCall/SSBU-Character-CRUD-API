...
ENTRYPOINT ["java", "-jar", "SsbuCharactersApplication.jar"]
...
# - ["java", "-jar", "SsbuCharactersApplication.jar"]: This uses the exec form (parsed as a JSON array) to run the exact command: java -jar SsbuCharactersApplication.jar.
# - java: Calls the Java runtime launcher.
# - -jar: Tells Java to execute a packaged JAR file.
# - SsbuCharactersApplication.jar: Points to the target file that was copied in the previous step.